...
#nafld matrix
nafld_ct_matrix <- ReadMtx(mtx = "C:/Users/kaylamac/Documents/Kayla's Code Stuff/Liver Atlas/NAFLD mouse/rawData_mouseNafld/countTable_mouseNafld/matrix.mtx",
                           features = "c:/Users/kaylamac/Documents/Kayla's Code Stuff/Liver Atlas/NAFLD mouse/rawData_mouseNafld/countTable_mouseNafld/features.tsv",
                           cells = "C:/Users/kaylamac/Documents/Kayla's Code Stuff/Liver Atlas/NAFLD mouse/rawData_mouseNafld/countTable_mouseNafld/barcodes.tsv",
                           feature.column = 1)


#another way to read in matrix (prob easier)
...
#subset for cell types we are interested in (n oimmune cells)
...
BiocManager::install(c("clusterProfiler", "org.Mm.eg.db", "msigdbr"))
...
msigdb_list <- msigdb_mouse %>% dplyr::select(gs_name, gene_symbol)


#create psuedobulk counts
#mimicks bulk RNA-seq data aggregating by celltype and diet
...
#create list to stor GSEA for each celltype_diet
gsea_all_results <- list()

#get unique cell types in your subset
celltypes <- unique(nafld_all_sub$annot)


#for loop to traverse through each celltype and pperform GSEA
for (celltype in celltypes) {
  celltype <- trimws(celltype)
  
  group1 <- paste0(celltype, "-WD")
  group2 <- paste0(celltype, "-SD")
  
  #check if both diet columns exist for this celltype
  if (!(group1 %in% colnames(pseudobulk_counts)) || !(group2 %in% colnames(pseudobulk_counts))) {
    message("Skipping ", celltype, ": missing ", group1, " or ", group2)
    next
  }
  
  #extract expression matrix for these two diet groups
  expr_mat <- pseudobulk_counts[, c(group1, group2)]
  
  #calculate log fold change (WD over SD)
  logFC <- log2((expr_mat[, group1] + 1) / (expr_mat[, group2] + 1))
  names(logFC) <- rownames(expr_mat)
  
  #sort gene list in decreasing order for GSEA
  ...
  if (!is.null(gsea_result) && length(gsea_result) > 0) {
    #save the result to the list
    gsea_all_results[[celltype]] <- gsea_result

    write.xlsx(as.data.frame(gsea_result), 
               file = paste0("GSEA_", gsub(" ", "_", celltype), ".xlsx"))
    
    message("GSEA done for ", celltype)
  } else {
    message("No significant GSEA results for ", celltype)
  }
}

#directory
output_dir <- "C:/Users/kaylamac/Documents/Kayla's Code Stuff/Liver Atlas/NAFLD mouse/GSEA_dotplots/"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)


#for loop to craete and save dotplot for each celltype
for (celltype in names(gsea_all_results)) {
  gsea_obj <- gsea_all_results[[celltype]]
  
  #skip if null or empty
  if (is.null(gsea_obj) || length(gsea_obj) == 0) {
    message("Skipping ", celltype, ": no GSEA result")
    next
  }
  
  #create the dotplot
  p <- dotplot(gsea_obj, showCategory = 10) + ggtitle(paste0(celltype, " GSEA"))
  
  print(p)
  
  ggsave(filename = paste0(output_dir, gsub(" ", "_", celltype), "_GSEA_dotplot.png"),
